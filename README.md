...
本项目的自动按键只通过 `PostMessage` 投递给已绑定窗口，不向任意前台窗口发送主按键。后台 keydown 的 `lParam` 保留随机扰动，keyup 保持标准语义；拟人化延迟、随机按压时长与节奏变化继续生效。Hook 内部用于释放修饰键或回放 Alt 状态的少量辅助事件仍会使用标准 `SendInput`，但不承载配置中的自动按键。
...
后台 `PostMessage` 是否有效由目标程序的消息处理方式决定；管理员权限程序通常要求本程序也以同等权限运行。窗口失效或 `PostMessageW` 调用失败时任务会停止，不会降级为向当前前台应用发送输入；程序无法判断目标是否实际消费了已成功投递的消息。
